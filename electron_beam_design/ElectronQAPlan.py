""" Create Electron QA Plan
    ...
    Creates a QA plan for an electron beam onto a water phantom in order to compare RayStation vs. Mobius MU.
    ...
    Validation Notes: The script relies on the status of the QA phantom being verifiable as read-only.  
                      Due to a programming error in RayStation 6.0 this status validation fails when 
                      QA plans are generated as part of a python script.  Therefore, unless the user 
                      has, without a script, validated the read-only status of the plan by opening a 
                      "dummy" QA plan, the script will fail to execute. 
    
    Version Notes: 1.0.0 Original
                   1.0.1 Added a relabeling of the plan to include the Beamset name to avoid ambiguity 
