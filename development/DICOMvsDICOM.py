...
import pydicom
import numpy as np
from DicomPairClasses import Result, ElementPair, SequencePair, DicomTreePair
...
def returnExpectedMismatch(value_pair, comment=""):
    return (Result.ELEMENT_EXPECTED_MISMATCH, comment)

def assessNearMatch(value_pair, comment="", tolerence_value=0.01):
    ...
    if np.allclose(ds1_array, ds2_array, rtol=0.0, atol=tolerence_value):
...
process_function_dict = {
    "SoftwareVersions": (returnExpectedMismatch, {}),
    "LeafJawPositions": (assessNearMatch, {"tolerence_value": 0.01}),  # 0.01 mm
    "TableTopLateralPosition": (returnExpectedMismatch, {}),
    "TableTopLongitudinalPosition": (returnExpectedMismatch, {}),
    "TableTopVerticalPosition": (returnExpectedMismatch, {}),
}
...
def create_dicom_tree_pair(ds1, ds2, depth=0):
    ...
    for ds1_keyword in ds1.dir():
        ...
        # to use for matching. Check match_dict to see if one is specified. If not, skip it.
        if ds1_keyword not in match_dict.keys():

            tree_list.append(
                SequencePair(
                    attribute_name=ds1_keyword,
                    sequence_list=[],
                    comment="Sequence was skipped because the attribute was not found in match_dict",
                    depth=depth+1,
                )
            )    
        ...
        if ds1_keyword not in ds2.dir():
            ...
            for item1 in ds1[ds1_keyword]:
                sequence_list.append(
                    create_dicom_tree_pair(item1,pydicom.Dataset(), depth=depth+2)
                )
        ...
        match_keyword = match_dict[ds1_keyword]
        ...
        for item1 in ds1[ds1_keyword]:
            ...
            for item2 in ds2[ds1_keyword]:
                if item1[match_keyword].value == item2[match_keyword].value:
                    ...
                    found = True
                    sequence_list.append(
                        create_dicom_tree_pair(item1, item2, depth=depth+2)
                    )
        ...
        for item2 in ds2[ds1_keyword]:
            ...
            if not found:
                # Unique to dataset 2
                sequence_list.append(create_dicom_tree_pair(pydicom.Dataset(), item2, depth=depth+2))
    ...
    for ds2_keyword in ds2.dir():
        ...
        if (ds2_keyword not in match_dict.keys()) and (ds2_keyword not in ds1.dir()):
        ...
        if ds2_keyword not in ds1.dir():
            ...
            for item2 in ds2[ds2_keyword]:
                sequence_list.append(
                    create_dicom_tree_pair(pydicom.Dataset(), item2, depth=depth+2)
                )
    ...
    return DicomTreePair(tree_list=tree_list, depth=depth)
...
print(compare_dicomrt_plans(file_path / raystation_filename, file_path / aria_filename))
