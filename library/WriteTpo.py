...
def pdf(patient, exam, plan, fields, target_priority=2, overwrite=True):
    ...
    canvas = Canvas(filename=f, pagesize=letter, pageCompression=1, verbosity=1, encrypt=None)
    frame = Frame(0.75 * inch, 0.75 * inch, 7 * inch, 9.5 * inch, showBoundary=0)
    s = ParagraphStyle({'fontName': 'Times',
                        'fontSize': 11,
                        'leading': 11,
                        'leftIndent': 0,
                        'rightIndent': 0,
        ...
        if fields['targets'][t]['use']:
            ...
            for n in range(len(fields['plans'])):
                if len(fields['targets'][t]['dose']) > n and fields['targets'][t]['dose'][n] > 0:
                    targets[-1].append(P('{} Gy'.format(fields['targets'][t]['dose'][n]), s))
    ...
    tables = [
        Table([['<image>', P('<b><font size=14>' + fields['protocol'] +
                             ' Treatment Planning Order</font></b>', s)]],
              colWidths=[1.5 * inch, 5 * inch],
              spaceAfter=0.25 * inch,
              hAlign='LEFT'),
        Table([[P('<b>Name:</b>', s), P(patient.Name.replace('^', ' '), s)],
               ...
               [P('<b>Planning Image:</b>', s), P('{}, {} {} ({}/{:02}/{} {:02}:{:02})'.
                                                  format(exam.Name,
                                                         ...
                                                         info['EquipmentModule']['ManufacturersModelName'],
              ...
              hAlign='LEFT'),
        Table(details,
              colWidths=[1.5 * inch] + [min(1.5, 4.5 / len(details[0])) * inch] * len(fields['plans']),
              spaceAfter=0.25 * inch,
              hAlign='LEFT',
        ...
        Table(targets,
              colWidths=[1.5 * inch] + [min(1.5, 4.5 / len(details[0])) * inch] * (len(fields['plans']) + 1) +
                        [0.75 * inch],
              spaceAfter=0.25 * inch,
              hAlign='LEFT',
              style=TableStyle([('BOX', (0, 0), (-1, -1), 0.25, colors.black),
    ...
    frame.addFromList(tables, canvas)
    canvas.save()
...
pdf(patient=connect.get_current('Patient'),
    exam=connect.get_current('Examination'),
    plan=connect.get_current('Plan'),
    fields={'protocol': 'UW Prostate',
            'order': 'Prostate Only',
            'institution': 'UWHC',
            'diagnosis': 'C61. Malignant neoplasm of prostate',
            'options': {'Previous radiation': True, 'CEID/Pacemaker': True},
            'fractions': [28, 16],
            'technique': ['VMAT', 'VMAT'],
            'frequency': ['Daily', 'Daily'],
            'imaging': ['CBCT', 'CBCT'],
            'motion': ['Fiducial Tracking', 'Fiducial Tracking'],
            'plans': ['Pros_VMA_R0A0', 'Pros_BS1_R0A0'],
            'targets': {'PTV': {'use': True, 'structure': 'PTV', 'dose': [50.4, 28.8]}}})
