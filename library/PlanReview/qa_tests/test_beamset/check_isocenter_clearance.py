...
def convert_to_contours(rso, roi_name):
    """
    Convert an ROI to contours and return the contour points.

    Args:
        rso: RayStation object containing beamset information.
        roi_name (str): Name of the ROI to convert to contours.
...
# ================= Geometry Utilities =================
def convert_angle_from_cylindrical_to_varian_iec(cylindrical_angle):
    """
    ...
    Args:
        cylindrical_angle (float): The angle in degrees in the cylindrical coordinate system.

    Returns:
        float: The corresponding angle in degrees in the Varian IEC coordinate system.
    """
    varian_iec_angle = (cylindrical_angle - 270) % 360
...
def rotate_roi(rso, roi_name, couch_angle):
    ...
    #     return True
    # except Exception as e:
    #     logging.warning(f"An error occurred during rotation: {e}")
    #     return False
...
def get_sorted_cylindrical_angles_dicom(rso, contours, beam_name):
    ...
    Args:
        rso: RayStation object containing beamset information.
        contours (list): A list of contours representing a single roi
            each a list of points as dictionaries with 'x', 'y', 'z' coordinates.
    ...
    x = rotated_points[:, 0]
    y = rotated_points[:, 1]
    dicom_cylindrical_angles = np.degrees(np.arctan2(y, x)) % 360
    # Remap these to Varian gantry angles:
    varian_angles = convert_angle_from_cylindrical_to_varian_iec(dicom_cylindrical_angles)
...
def check_isocenter_clearance(rso):
    ...
    Test Patient:
        ...
        FAIL: Case 2 Contains a deliberately distorted ExternalClean to flag an angle aropund 305 degrees.
    """
