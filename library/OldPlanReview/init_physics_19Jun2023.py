...
def get_beamset_level_tests(rso, physics_review=True, log_messages=None):
    ...
    beamset_checks_dict = {
        ...
        "Beamset Template Selection":
            (BeamSetReviewTests.parse_beamset_selection, {'LOG_MESSAGES': log_messages}),
...
def check_plan(physics_review=True):
    ...
    """
    patient_checks_dict = get_exam_level_tests(rso) if rso.exam else {}
    """
    Gather Plan Level Checks
    """
    plan_checks_dict = get_plan_level_tests(rso, physics_review) if rso.plan else {}
    """
    Gather BeamSet Level Checks
    """
    log_messages = retrieve_logs(rso) if rso.beamset else []
    beamset_checks_dict = get_beamset_level_tests(rso, physics_review, log_messages) if rso.beamset else {}
    """
    Parse logs
    """
    # Execute qa_tests
    ...
    tpo_dialog = parse_order_selection(beamset_name=rso.beamset.DicomPlanLabel,
    ...
    if log_messages:
        for m in log_messages:
            ...
            if m['Log_Level'] not in parent_nodes:
                treedata.Insert(log_key[0], m['Log_Level'], m['Log_Level'], "")
                parent_nodes.add(m['Log_Level'])
            treedata.insert(m['Log_Level'], f"{m['Date']} {m['Time']}",
