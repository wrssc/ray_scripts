...
from ReviewDefinitions import LOG_DIR, DEV_LOG_DIR, KEEP_PHRASES, DOMAIN_TYPE
...
def new_read_log_file(patient_id):
    """
...
def new_parse_log_file(lines, parent_key, phrases):
    """
...
def retrieve_logs(rso):
    ...
    if not rso.patient:
        return None
    lines = new_read_log_file(patient_id=rso.patient.PatientID)
    message_logs = new_parse_log_file(lines=lines, parent_key=log_key[0],
