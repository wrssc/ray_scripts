...
from PlanReview_V0_BetaTesting.utils import get_approval_info
...
def generate_doc(rso, tests, header_data, test_mode=False):
    ...
    if test_mode:
        ...
        output_beta = os.path.join(OUTPUT_DIR,"BetaTesting_Fall2023")
    else:
        output_beta = os.path.join(OUTPUT_DIR,"BetaTesting_Fall2023")
        dump_tests_to_json(tests, file_name=os.path.join(
            output_beta,f"{rso.patient.PatientID}_tests.json"))
        dump_tests_to_json(tests, file_name=os.path.join(
            output_beta,f"{rso.patient.PatientID}_header.json"))
    tests_df = read_data(tests)
    # Output file
    file_name = f"{rso.patient.PatientID}_{rso.beamset.DicomPlanLabel}.doc"
    output_file = os.path.join(output_beta, file_name)
