...
COUCH_SUPPORT_STRUCTURE_EXAMINATION = {
    "HFS": "Supine Patient",
    "HFP": "Prone Patient",
    "FFS": "Supine Patient",
    "FFP": "Prone Patient",
...
def verify_localization_point(case, exam):
    ...
    This function searches for two conditions:
...
def get_support_structures_GUI(examination):
    ...
    patient_orientation = examination.PatientPosition
    ...
    layout.append(
        [
            sg.Text(
                f'Please select support structures for examination "{examination.Name} ({patient_orientation})"'
            )
        ],
    )

    # Add Treatment Table Frame
    layout.append(
        [
            sg.Frame(
                layout=[
...
def expand_geometry_to_superior_boundary(examination, geometry):
    """Uses the ROI expansion function to expand structure to superior limit.

    PARAMETERS
    ----------
    examination : ScriptObject
        The examination on which the expansion will happen.
    geometry : ScriptObject
        The geometry that will be expanded.
    ...
    NOTES
    -----
    This function does not take the last slice and copy it to the limit of the scan.
    Instead, it uses the expand function, which will distort the shape of the geometry.
    As such, it should only be used for structures that are static in the
    ...
    if geometry.GetBoundingBox()[1]["z"] < extent_sup:
        ...
        while geometry.GetBoundingBox()[1]["z"] < extent_sup:
            ...
            geometry.OfRoi.CreateMarginGeometry(
                Examination=examination,
                SourceRoiName=geometry.OfRoi.Name,
                MarginSettings=MarginSettings,
            )

        # Perform a single contraction to match image boundaries
        contract_sup = geometry.GetBoundingBox()[1]["z"] - extent_sup
    ...
    # If superior edge of couch is outside image boundary
    elif geometry.GetBoundingBox()[1]["z"] > extent_sup:
        ...
        expand_sup = extent_sup - geometry.GetBoundingBox()[1]["z"]
    ...
    logging.info(
        f"Successfully matched the {geometry.OfRoi.Name} to superior image extent."
    )


def expand_geometry_to_inferior_boundary(examination, geometry):
    """Uses the ROI expansion function to expand structure to inferior limit.
    ...
    if geometry.GetBoundingBox()[0]["z"] > extent_inf:
        ...
        while geometry.GetBoundingBox()[0]["z"] > extent_inf:
            MarginSettings = {
                "Type": "Expand",
                "Superior": 0,
                "Inferior": 15,
    ...
    elif geometry.GetBoundingBox()[0]["z"] < extent_inf:
        # Contract couch until it is within image boundary
        while geometry.GetBoundingBox()[0]["z"] < extent_inf:
            MarginSettings = {
                "Type": "Contract",
    ...
    logging.info(
        f"Successfully matched the {geometry.OfRoi.Name} to inferior image extent."
    )


def deploy_couch_model(
    case, support_structure_template, support_structures_examination, source_roi_names
):
    ...
    support_structure_template: str
        The name of the support structure template in RayStation
    support_structures_examination: dict of str
        The name of the examination associated with support_structure_template
        from which you will load the source ROIs
    source_roi_names : dict o dict of str
        The names of the ROIs from support_structures_examination that you wish
        to load)
    ...
    # This script is designed to be used for HFS or HFP patients:
    examination = get_current("Examination")
    patient = get_current("Patient")

    if examination.PatientPosition not in ["HFS", "HFP", "FFS", "FFP"]:
        logging.error(
            "Current exam in not in HFS, HFP, FFS or FFP position. Exiting script."
        )
        message = (
            "The script requires a patient in the HFS, HFP, FFS or FFP position. "
            "Exiting script."
        )
    ...
    with CompositeAction("Shift Couch Model"):
        ...
        if examination.PatientPosition == "HFS":
            ...
            top_of_couch = couch.GetBoundingBox()[1]["y"]
        elif examination.PatientPosition == "FFS":
            top_of_couch = couch.GetBoundingBox()[0]["y"]
    ...
    position_after_manual_change = couch.GetCenterOfRoi()

    log_string = "STRUCTURE POSITION CORRECTION LOG: "
    log_string += f"{patient.PatientID}, "
    log_string += f"{examination.Name}, "
    log_string += f"{examination.PatientPosition}, "
    log_string += (
        f"{examination.GetAcquisitionDataFromDicom()['SeriesModule']['ProtocolName']}, "
    )
    log_string += f"{examination.GetAcquisitionDataFromDicom()['SeriesModule']['SeriesDescription']}, "
    log_string += f"{examination.GetAcquisitionDataFromDicom()['SeriesModule']['SeriesDateTime']}, "
    log_string += f"{couch.OfRoi.Name}, "
    log_string += f"{position_before_manual_change}, "
    log_string += f"{position_after_manual_change}, "

    diff = [
        position_after_manual_change["x"] - position_before_manual_change["x"],
        position_after_manual_change["y"] - position_before_manual_change["y"],
        position_after_manual_change["z"] - position_before_manual_change["z"],
    ]
    log_string += f"{diff}"
...
def deploy_civco_breastboard_model(
    case, incline_angle, use_wingboard, wingboard_index, couch=None
):
    ...
    with CompositeAction("Apply Manual Corrections"):
        ...
        for roi in initial_shifts_rois:
            transform_structure(
                examination=examination, geometry=roi, translations=manual_translation
            )
    ...
    with CompositeAction("Incline and Shift Wingboard"):
        ...
        if use_wingboard:
            ...
            message = f"Civco Monarch wingboard moved to index {wingboard_index}."
            logging.info(message)
    ...
    with CompositeAction("Create Final ROIS (if needed)"):
        ...
        if not exists_roi(case, "NoFlyZone_PRV")[0]:
            case.PatientModel.CreateRoi(
                Name="NoFlyZone_PRV", Color="Green", Type="Avoidance"
            )
    ...
    with CompositeAction("Set Incline Board to Wax/Cork combo"):
        ...
        try:
            ...
            assert (
                material_wax is not None
            ), "Wax material was not found in Patient Model."
            ...
            assert (
                material_wax is not None
            ), "Wax material was not found in Template Materials."
        ...
        try:
            ...
            assert (
                material_cork is not None
            ), "Cork material was not found in Patient Model."
            ...
            assert (
                material_cork is not None
            ), "Cork material was not found in Template Materials."
