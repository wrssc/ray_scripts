...
def deploy_couch_model(
    ...
    if (examination.PatientPosition != "HFS") or (examination.PatientPosition != "HFP"):
        logging.error("Current exam in not in HFS or HFP position. Exiting script.")
        message = (
            "The script requires a patient in the HFS or HFP position. "
            "Exiting script."
        )
    ...
    with CompositeAction("Fill Couch Model Longitudinally"):
        ...
        # If superior edge of couch is outside image boundary
            ...
            logging.info("Successfully matched the treatment couch to image extent.")
    ...
    log_string += f"{examination.GetAcquisitionDataFromDicom()["SeriesModule"]["ProtocolName"]}, "
    ...
    log_string += f"{position_after_manual_change-position_before_manual_change}"
