...
def output_message(status):
    """Returns message for write out"""
    output_message = \
          status['PatientID'] + "\t" \
        + status['Case'] + "\t" \
        + status['PlanName'] + "\t" \
        + status['BeamSetName'] + "\t" \
        + status['PlanningStructures_Created'] + "\t" \
        + status['Beams_Loaded":'] + "\t" \
        + status['ClinicalGoals_Loaded'] + "\t" \
        + status['PlanOptimization_Loaded'] + "\t" \
...
def main():
    ...
    output_message = "PatientID" \
               ...
               + "\tPlanName" \
               + "\tBeamSetName" \
               + "\tPlanningStructures_Created" \
               + "\tBeams_Loaded"\
               + "\tClinicalGoals_Loaded"\
               + "\tPlanOptimization_Loaded"\
    ...
    for index, row in plan_data.iterrows():
        ...
        patient_info = db.QueryPatientInfo(
            Filter={
                ...
                'PatientID': '^{0}$'.format(row.PatientID)
            }
        )
