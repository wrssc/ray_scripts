...
def compute_comparison_statistics(patient, case, exam, rois_a, rois_b, compute_distances=False):
    ...
    for r in exist_list_b:
        if r:
            ...
            roi_index += 1
    logging.debug(
        "Found the following in evaluation of overlap with {} and {} ".format(rois_verified_a, rois_verified_b)
    )
...
def iter_standard_rois(etree):
    ...
    for r in etree.iter('roi'):
        ...
        for d in derived:
            ...
            try:
                logging.debug('Source {} is with type {}'.format(
                    r.find(source_key).text,
                    type(r.find(source_key).text)
                    )
                              )
...
def create_derived(patient, case, examination, roi, df_rois, roi_list=None):
    ...
    if not df_needs_derived.empty:
        for index, row in df_needs_derived.iterrows():
            ...
            logging.debug('{} has all dependencies. Checking derived status'.format(derived_roi_name))
