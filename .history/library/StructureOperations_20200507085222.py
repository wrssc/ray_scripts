...
def create_prv(patient, case, examination, source_roi, df_TG263):
    ...
    if not df_prv.empty:
        ...
        # ab = time_b -time_a
        # bc = time_c - time_b
        # logging.debug('Check struct {} versus exists {}'.format(ab.total_seconds(),bc.total_seconds()))
...
def planning_structures(
    ...
    roi_approved = structure_approved(case=case,structure_name=StructureName,
                                      examination=examination)
    if roi_approved:
        UserInterface.WarningBox('Examination {} is approved'.format(examination.Name)
                                 + ' Select an examination on without approved structures'
