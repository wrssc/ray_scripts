...
def check_derivation(patient, case, examination, **kwargs):
    ...
    current_expression = test_structure.DerivedRoiExpression
    # If the current expression is None, the structure has no derived status
    if current_expression is None:
        return False
    if current_expression.ExpandContractType != MarginTypeR:
        return False
    if current_expression.SuperiorDistance != ExpR[0]:
        return False
    if current_expression.InferiorDistance != ExpR[1]:
        return False
    if current_expression.AnteriorDistance != ExpR[2]:
        return False
    if current_expression.PosteriorDistance != ExpR[3]:
        return False
    if current_expression.RightDistance != ExpR[4]:
        return False
    if current_expression.LeftDistance != ExpR[5]:
        return False
    # Check A/B operation
    if current_expression.Children[0].Operation != OperationResult:
        return False
    # Check the expansion type and distances for A
    current_a_expression = current_expression.Children[0].Children[0]
    if current_a_expression.ExpandContractType != MarginTypeA:
        return False
    if current_a_expression.SuperiorDistance != ExpA[0]:
        return False
    if current_a_expression.InferiorDistance != ExpA[1]:
        return False
    if current_a_expression.AnteriorDistance != ExpA[2]:
        return False
    if current_a_expression.PosteriorDistance != ExpA[3]:
        return False
    if current_a_expression.RightDistance != ExpA[4]:
        return False
    if current_a_expression.LeftDistance != ExpA[5]:
        return False
    # Check the expansion type and distances for B
    current_b_expression = current_expression.Children[0].Children[1]
    if current_b_expression.ExpandContractType != MarginTypeB:
        return False
    if current_b_expression.SuperiorDistance != ExpB[0]:
        return False
    if current_b_expression.InferiorDistance != ExpB[1]:
        return False
    if current_b_expression.AnteriorDistance != ExpB[2]:
        return False
    if current_b_expression.PosteriorDistance != ExpB[3]:
        return False
    if current_b_expression.RightDistance != ExpB[4]:
        return False
    if current_b_expression.LeftDistance != ExpB[5]:
        return False
