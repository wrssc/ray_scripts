        ...
        if current_dicom_type == 'External':
            ...
            try:
                ...
                error_message.append(
                    'External error. Could not change {} to type {}'.format(roi_name, roi_type))
        return error_message
    ...
    for k, v in type_dict.items():
        if any(roi_type in types for types in v):
            if current_organ_type == k:
                error_message.append(
                    "Structure {} is already organ type {}".format(roi_name, current_organ_type))
            else:
                try:
                    rs_roi.OrganData.OrganType = k
                except:
                    error_message.append(
                        "Type error. Could not change {} to type {}".format(roi_name, roi_type))
    return error_message
...
def compute_comparison_statistics(patient, case, exam, structure, rois):
    """
    ...
    if len(rois_verified) > 1:
        ...
        stats = case.PatientModel.StructureSets[exam.Name].ComparisonOfRoiGeometries(RoiA=structure, RoiB=union_name)
        case.PatientModel.RegionsOfInterest[union_name].DeleteRoi()
    else:
        stats = case.PatientModel.StructureSets[exam.Name].ComparisonOfRoiGeometries(RoiA=structure, RoiB=rois_verified)
    # vol = None
    # try:
    #     t = case.PatientModel.StructureSets[exam.Name].RoiGeometries[overlap_name]
    #     if t.HasContours():
...
def make_externalclean(
    ...
    while check_structure_exists(case=case,structure_name=temp_ext,exam=examination,option="Check"):
        i += 1
        temp_ext = 'temp_ext'+str(i)
