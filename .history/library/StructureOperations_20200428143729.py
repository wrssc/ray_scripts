...
def make_boolean_structure(patient, case, examination, **kwargs):
	...
	if StructureName == "InnerAir":
		logging.debug('A sources {}'.format(SourcesA))
		logging.debug('B sources {}'.format(SourcesB))
...
def planning_structures(
	...
	initial_target_number = planning_structure_selections.first_target_number
	...
	if dialog2_response is not None:
		...
		for i in range(1, number_of_targets + 1):
			j = str(i + initial_target_number - 1)
