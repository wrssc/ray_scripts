...
def iter_standard_rois(etree):
    ...
    for r in etree.iter('roi'):
        ...
        try:
            roi["Dependencies"] = str(r.find('Dependencies').text).split(",")
        except AttributeError:
...
def create_derived(patient, case, examination, roi, df_TG263, roi_list=None):
    """
    ...
    df_e = df_TG263[df_TG263.Dependencies.isin(roi)]
    if not df_e.empty:
