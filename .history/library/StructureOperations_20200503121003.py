...
def check_derivation(patient, case, examination, **kwargs):
    ...
    if current_a_expression.ExpandContractType != MarginTypeA:
        return False
    if current_a_expression.ExpandContractType != MarginTypeR:
        return False
    if current_a_expression.SuperiorDistance != ExpR[0]:
        return False
    if current_a_expression.InferiorDistance != ExpR[1]:
        return False
    if current_a_expression.AnteriorDistance != ExpR[2]:
        return False
    if current_a_expression.PosteriorDistance != ExpR[3]:
        return False
    if current_a_expression.RightDistance != ExpR[4]:
        return False
    if current_a_expression.LeftDistance != ExpR[5]:
        return False
