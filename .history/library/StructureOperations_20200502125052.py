...
def change_roi_type(case, roi_name, roi_type):
    ...
    if not all(exists_roi(case=case, rois=roi_name)):
        error_message.append("Structure {} not found on case {} \n".format(roi_name, case))
        return error_message
    rs_roi = case.PatientModel.RegionsOfInterest[roi_name]
    ...
    if roi_type == 'External':
        if current_dicom_type == 'External':
            error_message.append("Structure {} is already type {}\n".format(roi_name, roi_type))
        else:
            try:
                rs_roi.SetAsExternal()
            except:
                error_message.append('External error. Could not change {} to type {}\n'.format(roi_name, roi_type))
        return error_message
    ...
    if current_dicom_type == roi_type:
        ...
        try:
            ...
            error_message.append("Unable to change type on roi {} from Dicom type {} to type {}\n".format(
